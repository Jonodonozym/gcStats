...
public class AchievementData {
		...
		new BukkitRunnable() {
			...
			public void run() {
				for (String server : Config.servers) {
					List<String> statTypes = StatsDatabase.getInstance().getEnabledStats(server);
					statTypes.remove("UUID");
					for (String type : statTypes) {
						double value = StatsDatabase.getInstance().getStat(p, type.toString(), server);
						for (Achievement a : achievementsByType.get(server).get(type))
							if (a.isAchieved(value))
								StatsDatabase.getInstance().setAchieved(p, a);
					}
				}
			}
		}.runTaskAsynchronously(GCStats.plugin);
	}

	public static void reloadData(JavaPlugin plugin) {
		// loading from the local file
		String location = GCStats.plugin.getDataFolder().getPath() + File.separator + "Achievements.yml";
		File file = new File(location);
		if (!file.exists())
			new FileExporter(plugin).ExportResource("Achievements.yml", location);
		...
		for (String achievement : achConfig.getConfigurationSection("achievements").getKeys(false)) {
			try {
				...
				if (!Config.enabledStats.contains(type)){
					GCStats.plugin.getLogger().info("achievement " + achievement + " stat type is disabled in config.yml, skipping...");
					continue;
				}
				...
				GCStats.plugin.getLogger().info("achievement " + achievement + " has invalid configuration, skipping...");
			}
		}

		// updating the sql db to match
		StatsDatabase.getInstance().ensureCorrectAchMetaTable(localAchievements);
		StatsDatabase.getInstance().ensureCorrectAchTable(localAchievements);
		...
		for (Achievement a : StatsDatabase.getInstance().getAllAchievements()) {
	...
	public static void updateAchievements(Player p, StatType s) {
		try{
			double value = StatsDatabase.getInstance().getStat(p, s.toString());
